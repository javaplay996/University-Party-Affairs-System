<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="户籍" prop="huji">
          <el-input v-model="ruleForm.huji" 
              placeholder="户籍" clearable  :readonly="ro.huji"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="户籍" prop="huji">
              <el-input v-model="ruleForm.huji" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="材料类型" prop="cailiaoleixing">
          <el-select :disabled="ro.cailiaoleixing" v-model="ruleForm.cailiaoleixing" placeholder="请选择材料类型">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="材料类型" prop="cailiaoleixing">
	      <el-input v-model="ruleForm.cailiaoleixing"
                placeholder="材料类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.cailiaoneirong" label="材料内容" prop="cailiaoneirong">
          <file-upload
          tip="点击上传材料内容"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.cailiaoneirong?ruleForm.cailiaoneirong:''"
          @change="cailiaoneirongUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.cailiaoneirong" label="材料内容" prop="cailiaoneirong">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.cailiaoneirong)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chushengnianyue = true;
          ...
          if(o=='cailiaoleixing'){
            this.ruleForm.cailiaoleixing = obj[o];
	    this.ro.cailiaoleixing = true;
            continue;
          }
          if(o=='cailiaoneirong'){
      ...
      this.$http({
		...
		if(json.huji!=''&&json.huji){
                this.ruleForm.huji = json.huji
	    		this.ro.huji = true;
		}
		if(json.chushengnianyue!=''&&json.chushengnianyue){
                this.ruleForm.chushengnianyue = json.chushengnianyue
	    		this.ro.chushengnianyue = true;
		}
        } else {
	...
	if(this.ruleForm.cailiaoneirong!=null) {
		this.ruleForm.cailiaoneirong = this.ruleForm.cailiaoneirong.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `rudangziliao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.rudangziliaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    cailiaoneirongUploadChange(fileUrls) {
	this.ruleForm.cailiaoneirong = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
